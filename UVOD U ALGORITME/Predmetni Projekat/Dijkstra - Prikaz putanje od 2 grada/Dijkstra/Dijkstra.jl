...
function Dijkstra!(Graf::Graf, sekundarniKljuč::Int32, KrajnjiGrad::Int32)    
    ...
    početneVrednosti!(Graf, Graf.V[sekundarniKljuč]) 
                                            # Početni čvor ima vrednost udaljenosti koja je 0, dok svi
    ...
    while length(Q) > 0
        ...
        u.boja = B
        if u.ključ == KrajnjiGrad
            if u.udaljenost == Inf
                println("GRAD NIJE POVEZAN!")
                break
            end
            tekst1  = split(Graf.V[sekundarniKljuč].ImeGrada, " ")
            tekst2  = split(Graf.V[u.ključ].ImeGrada, " ")
